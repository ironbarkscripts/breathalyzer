...
4. **Before production deployment, set `Config.DevMode = false`**
...
`Config.DevMode = true` adds a `qb-target` option on all nearby NPCs labelled **Breath Test [DEV]**. The server generates a random BAC (0.00–0.25) and runs the full UI flow. No money changes hands and no job consequences apply. The result notification is prefixed with `[DEV] NPC Test`.
...
| `DevMode` | `true` | NPC testing; disable before go-live |
